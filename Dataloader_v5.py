...
def data_loader(index, root, mode, Transform_data, Transform_lbl, ctg_index, batch_size):
    ...
    def Categories(input_image, input_list, pix_num_arr):
        input_list.sort()
        C = len(input_list)
        ...
        for i in range(C):
            ...
            pix_num_arr[pixel_value] += np.sum(input_image == input_list[i])
            
        target = np.array(target)
        
        return Image.fromarray(np.uint8(target)), pix_num_arr # pix_num_lst is a 8*1 array
    ...
    for i in range(int(len(index) / batch_size)):
        ...
        pix_num_arr = np.zeros((8,1))
        pix_num_out = np.zeros((8,1))
        ...
        for j in range(batch_size):
            ...
            lbl_idx, pix_num_arr = Categories(lbl_idx, ctg_index, pix_num_arr)
        ...
        for idx in range(pix_num_arr.shape[0]):
            if pix_num_arr[idx,0] == 0:
                pix_num_out[idx,0] = 0
            else:
                pix_num_out[idx,0] = 1 / pix_num_arr[idx,0]

        yield data_out, lbl_out.long(), data_ori, pix_num_out